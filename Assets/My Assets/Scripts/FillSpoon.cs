...
public class FillSpoon : MonoBehaviour
{
    ...
    [SerializeField] ActiveCoffeMesh filterScript;
    ...
    void DropCoffe()
    {
        ...
        if (isRotated && isCoffeActive)
        {
            ...
            if (Physics.Raycast(coffe.transform.position, Vector3.down, out hit, 10f))
            {
                ...
                {
                    //deveria corrigir isso para chamar uma função externa do proprio filtro
                    filterScript.ActiveCoffe();
                }
            }
